...
import type { CategoryType, Task, DayTemplate, Subtask } from './types';
...
const AppContent = () => {
  ...
  // Local UI state
  // Local UI state (separate from global state)
  ...
  const dayTasks = trackData[dateStr] || createEmptyDay(dailyThemes[dateStr]?.theme || 'none');
  const dayWastedLogs = wastedLogs[dateStr] || [];
  
  // Calculate temporal stats using service
